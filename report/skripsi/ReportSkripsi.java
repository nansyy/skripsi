...
public class ReportSkripsi extends Report implements ConnectionListener {

    protected HashMap<ReportSkripsi.ConnectionInfo, ConnectionInfo> connections;
    ...
    protected void init() {
        super.init();
        this.connections = new HashMap<ReportSkripsi.ConnectionInfo, ConnectionInfo>();
        this.nrofContacts = new Vector<Integer>();
    }

    protected void addConnection(DTNHost host1, DTNHost host2) {
      ReportSkripsi.ConnectionInfo ci = new ReportSkripsi.ConnectionInfo(host1, host2);

        //jika di connections ga ada key ci
        //true -> tambahkan koneksi baru
        //false -> tampilkan pesan error
    ...
    protected ConnectionInfo removeConnection(DTNHost host1, DTNHost host2) {
		 /*
            ...
            h2 = host2
            start time koneksi
            endtime = -1 (koneksi masih berlangsung)
         */
        ConnectionInfo ci = new ConnectionInfo(host1, host2);

        //method remove() dalam Java Collections, akan return nilainya SEBELUM dihapus
        //kalo dalam connections ga ada h1 dan h2 yg sama dengan ci, maka return null
        //return null berarti untuk saat ini ga ada koneksi aktif antara host1 dan host2
        ci = connections.remove(ci); //hapus koneksi yang key-nya ci lalu return informasi koneksi yang dihapus
        return ci;
    }
    ...
    protected class ConnectionInfo {
        ...
        public double getConnectionTime() {
            if (this.endTime == -1) {//koneksi masih berlangsung
                return getSimTime() - this.startTime;//berapa lama koneksi sudah berjalan sampai saat ini
            }
            else {//koneksi sudah berakhir
                return this.endTime - this.startTime;//total durasi koneksi
            }
        }
        ...
        public boolean equals(Object other) {
            if (!(other instanceof ReportSkripsi.ConnectionInfo)) {
                return false;
            }

            ReportSkripsi.ConnectionInfo ci = (ReportSkripsi.ConnectionInfo)other;
